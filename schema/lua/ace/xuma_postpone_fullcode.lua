...
local function is_match_short_or_only_code(cand, env)
  ...
  if not env.engine.context:get_option("xuma_postpone_fullcode") then
    ...
    for cand in input:iter() do
      if done_drop then
        ...
        local lift, is_exact_match = is_match_short_or_only_code(cand, env)
        if pos >= max_pos or not is_exact_match then
