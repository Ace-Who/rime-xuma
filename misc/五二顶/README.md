...
- 所有三码词、四码词，必须使用一类上屏法。
- 所有一码词、二码词，可以使用一类上屏法，也可以用上屏码（后详）上屏。
- 二码首选，若未达句末，可以省略空格，继续输入后续文字，每逢五码或上屏码会顶头二码的首选上屏。
- 二码首选，若省略空格，且后接一码词，共两词三码，不足五码，则须输入后词的上屏码，同时会顶前词上屏。
- 二码首选，若省略空格，且后接二码词，且已达句末，共两词四码，不足五码，则须输入后词的上屏码，同时会顶前词上屏。

总之，要打一段文字，仅三码以上需加空格或用标点顶屏；除末词外，对于首选词，一码
和二码都是二键（一码首选必加上屏码）；末词若为二码首选，则需加上屏码。即对于首
选词，五二顶将三键词变成二键词，正如五四顶（常规方案）将五键词变成四键词。

### 上屏码（旧称「区重补码」）

**上屏码**为一码词和二码词专用。有 `[ ; '` 三个上屏码，依次对应三重候选。例如
`u` 的三重候选依次为 「的／的是／的时候」，除了共享一个编码 `u`，它们还各自具有
唯一编码，依次为 `u[  u;  u'` ，输入这组编码将会直接上屏。为指法计，首选上屏码
`[` 还绑定到了 `TAB` 键，这样安排在双手对称位置，可避免同手连击和小指大移位的同
时发生。
...
固然可以定义符号键选重，但由于顶功占用了符号来作上屏码，就统一为数字键选重，以
免混淆和误解。）试比较它们的键长。
